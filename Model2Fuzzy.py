...
def CreateLabels(model, dataframe, colName ,labels, dataframeOutputName):
    print("Creating labels")
    df = dataframe.drop_duplicates().reset_index(drop=True) #Elimiate duplicates and indexes
   ...
   # Cycle to go through each tweet in the dataframe
    ...
    new_df.to_csv(dataframeOutputName, index=False)
...
def Fuzzy2Result(ExpData, RealData, labels, dataframeName):
    ...
    fuzzyDataFrame.to_csv(dataframeName, index=False)
